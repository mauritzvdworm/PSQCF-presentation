...
- <a href = "https://www.cftc.gov/sites/default/files/2019-04/ATS_2yr_Update_Final_2018_ada.pdf" target="_blank">Automated Trading in Futures Markets - Update 2, Richard Haynes and John S. Roberts</a>

- <a href = "https://www.cftc.gov/sites/default/files/2019-03/automatedordersreport032719.pdf" target="_blank">Impact of Automated Orders in Futures Markets, CFTC</a>
...
tradeData <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Start Ideas/PS trend/tradeData.csv"))
...
contPrices <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/CalendarPortfolio/contCalendarFracDiff.csv"))
...
fracdiffData <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/CalendarPortfolio/calendarFractionalDifferences.csv"))
...
BB1returns <- as.data.table(read_csv('C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/BB1portfoioreturns.csv'))
BB1returns[!is.na(return), cumret := cumsum(return)]

BB1live <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/BB1returns.csv"))
BB1live[, X1 := NULL]
setnames(BB1live, "index", "date")


BB1PSQCF <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/Quant/BB1returns.csv"))
BB1PSQCF[, X1 := NULL]
...
BB2returns <- as.data.table(read_csv('C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/calendarPortfolio_EWMA_RSI.csv'))
BB2returns[!is.na(return), cumret := cumsum(return)]

BB2live <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/BB2returns.csv"))
BB2live[, X1 := NULL]
setnames(BB2live, "index", "date")


BB2PSQCF <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/Quant/BB2returns.csv"))
BB2PSQCF[, X1 := NULL]
...
#monthlydata <- as.data.table(monthlydata)
monthlydata <- BB2returns[,.(normalised = prod(return + 1) - 1), by = .(year, month)]
monthlydata[, sign := sign(normalised)]
...
- <a href = "https://www.amazon.com/Advances-Financial-Machine-Learning-Marcos-ebook/dp/B079KLDW21" target="_blank">Advanced in financial machine learning - Marcos López de Prado</a>
...
allPrices <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Start Ideas/PS trend/designing strategies/allPrices.csv"))
...
returns_instrument_forecast_divmult_scale_weight_estimates <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Start Ideas/PS trend/designing strategies/returns_instrument_forecast_divmult_scale_weight_estimates.csv"))
...
TR1returns <- as.data.table(read_csv('C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/trendPortfolioReturns.csv'))
...
TR1live <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/TR1returns.csv"))
TR1live[, X1 := NULL]
setnames(TR1live, "index", "date")


TR1PSQCF <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/Quant/TR1returns.csv"))
TR1PSQCF[, X1 := NULL]
...
portfolio[date == max(date), management := 0]
...
portfoliolive <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/portfolioReturnsQCF.csv"))
...
portfoliolive[, year := year(date)]
portfoliolive[, month := month(date)]
#portfoliolive[, half := ifelse(month < 7, 1, 2)]

portfoliolive[, md := max(date), by = .(year, month)]

portfoliolive[, management := 0]
portfoliolive[date == md, management := 0.02/12]
portfoliolive[date == max(date), management := 0]

portfoliolive[, totalreturn := PnL - management]


portfoliolive <- portfoliolive[,.(date, PnL = totalreturn, type)]
...
portfolioReturns <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/portfolioReturnsQCF.csv"))
...
pic <- ggplot(portfolioReturns, aes(x=date, y=value, group = flag), 
...
plotdata[, month := month(date)]
plotdata[, day := substring(date, 9,10)]
plotdata[, year := year(date)]
plotdata[, newdate := paste("2000", month, day, sep = "-")]
plotdata[, newdate := as.Date(newdate)]
plotdata[, normalised := cumprod(1 + return), by = year]


meandata <- plotdata[, .(return = cumprod(1+return) - 1), by = .(month, year)]
meandata <- meandata[, .(return = mean(return)), by = month]
meandata[, newdate := paste(2000, month, 28, sep="-")]
meandata[, newdate := as.Date(newdate)]
start <- data.table(month = 0, return = 0, newdate = as.Date("2000-01-01"))
meandata <- rbind(start, meandata)
meandata[, normalised := cumprod(1 + return)]

pic <- ggplot(plotdata[year > 2000 & year != max(year) & !year %in% c(2003, 2008)], aes(x=newdate, y=normalised)) +
  geom_line(aes(group = year), alpha = 0.3) +
  geom_line(data = plotdata[year == max(year)], color = "steelblue", size = 1.2) +
  geom_line(data = meandata, color = "firebrick", linetype = "dashed") +
  geom_line(data = plotdata[year %in% c(2008, 2003)], aes(color = factor(year))) + 
  scale_x_date(labels = date_format("%b"), breaks = pretty_breaks(n=10)) +
  geom_hline(yintercept = 1) +
  xlab("") +
  ylab("Value of $1") +
  ggtitle("Yearly Performance of $1 invested on 1st of Jan") +
  theme(
    plot.background = element_rect(fill = rgb(242,242,242, maxColorValue = 255)),
    panel.background = element_rect(fill = "white"),
    panel.grid.major.y = element_line(size = 0.5, linetype = 'solid',colour = "gray"),
...
avgneg <- monthlydata[sign == -1 & index >= "2018-06-01", mean(cumret) * 100]
avgpos <- monthlydata[sign == 1 & index >= "2018-06-01", mean(cumret) * 100]

noneg <- nrow(monthlydata[sign == -1 & index >= "2018-06-01"])
nopos <- nrow(monthlydata[sign == 1 & index >= "2018-06-01"])
...
snp <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/spx.csv"))
...
quant <- tbldata[, .(date, PSQCF = cumret)]
...
together <- as.data.table(read_csv("C:/Box/Polar Star - Company Folder/1. Polar Star/Analysis/Analysis and Research/code/Quant Portfolio/data/together.csv"))
