...
ggplot(plotdata[date >= "2018-07-01"], aes(x=date, y= N)) +
  ...
  ggtitle("Number of unique commodities traded in each fund since July") +
...
plotdata <- rbind(BB1returns[date < BB1live[, min(date)],.(date, return, type = "Backtest")], BB1live[,.(date, return = PnL, type = "Live")])
...
plotdata <- rbind(BB2returns[date < BB2live[, min(date)],.(date, return, type = "Backtest")], BB2live[,.(date, return = PnL, type = "Live")])
plotdata[!is.na(return), cumret := cumsum(return)]
...
tbl1live <- table.AnnualizedReturns(BB1live[, .(date, `BB1 live` = PnL)])
nms <- row.names(tbl1live)
tbl1live <- as.data.table(tbl1live)
tbl1live[, variable := nms]

BB1live[, month := month(date)]
BB1live[, year := year(date)]
BB1live[, md := min(date), by = .(year, month)]

BB1live[is.na(PnL), PnL := 0]
BB1live[, normalised := cumprod(PnL + 1)]

monthlydata <- CalculateReturns(BB1live[date == md,.(date, normalised)])
monthlydata <- as.data.table(monthlydata)
monthlydata[, sign := sign(normalised)]
...
maxdraw <- maxDrawdown(as.xts(BB1live[, .(date, PnL)]))[1] * 100
...
tbl2live <- table.AnnualizedReturns(BB2live[, .(date, `BB2 live` = PnL)])
nms <- row.names(tbl2live)
tbl2live <- as.data.table(tbl2live)
tbl2live[, variable := nms]

BB2live[, month := month(date)]
BB2live[, year := year(date)]
BB2live[, md := min(date), by = .(year, month)]

BB2live[is.na(PnL), PnL := 0]
BB2live[, normalised := cumprod(PnL + 1)]

monthlydata <- CalculateReturns(BB2live[date == md,.(date, normalised)])
monthlydata <- as.data.table(monthlydata)
monthlydata[, sign := sign(normalised)]
...
maxdraw <- maxDrawdown(as.xts(BB2live[, .(date, PnL)]))[1] * 100
...
plotdata <- rbind(TR1returns[date < TR1live[, min(date)],.(date, return, type = "Backtest")], TR1live[,.(date, return = PnL, type = "Live")])
plotdata[!is.na(return), cumret := cumsum(return)]
...
tbl1live <- table.AnnualizedReturns(TR1live[, .(date, `TR1 live` = PnL)])
nms <- row.names(tbl1live)
tbl1live <- as.data.table(tbl1live)
tbl1live[, variable := nms]

TR1live[, month := month(date)]
TR1live[, year := year(date)]
TR1live[, md := min(date), by = .(year, month)]

TR1live[is.na(PnL), PnL := 0]
TR1live[, normalised := cumprod(PnL + 1)]

monthlydata <- CalculateReturns(TR1live[date == md,.(date, normalised)])
monthlydata <- as.data.table(monthlydata)
monthlydata[, sign := sign(normalised)]
...
maxdraw <- maxDrawdown(as.xts(TR1live[, .(date, PnL)]))[1] * 100
...
portfoliolive <- portfoliolive[,.(date, PnL = PSQCF)]
...
plotdata <- rbind(portfolio[date < portfoliolive[, min(date)],.(date, return = totalreturn, type = "Backtest")], portfoliolive[,.(date, return = PnL, type = "Live")])
plotdata[!is.na(return), cumret := cumsum(return)]
...
portfolioReturns[, totalreturn := PSQCF - management]
...
portfolioReturns <- portfolioReturns[, .(date, PSQCF = normalized, `S&P` = SnPnormalised)]
portfolioReturns <- melt(portfolioReturns, id.vars=c("date"))

#rebaldates <- as.Date(c("2018-07-03", "2018-08-02", "2018-09-05", "2018-10-02", "2018-11-02", "2018-12-02"))

ggplot(portfolioReturns, aes(x=date, y=value)) +
...
tbldata <- rbind(portfolio[,.(date, return = totalreturn)], portfoliolive[, .(date, return = PnL)])
...
tbl4 <- table.AnnualizedReturns(tbldata[date >= "2018-06-01", .(date, live = return)])
nms <- row.names(tbl4)
...
maxdraw <- maxDrawdown(as.xts(tbldata[date >= "2018-06-01", .(date, return)]))[1] * 100
