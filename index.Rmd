...
getPerformance <- function(df, hurdle = 1.02){
  #df = s.data[[1]]
  ret <- df[, prod(1 + totalreturn)]
  diff <- ret - hurdle
  perf <- ifelse(diff > 0, diff, 0) * 0.2
...
allPortfolios <- together[, .(date, `S&P500`, PSQCF, `PSQCF and S&P500` = 0.2 * PSQCF + 0.8 * `S&P500`)]
...
allPortfolios <- together[, .(date, `40-60` = PSLtd * 0.6 + PSQCF * 0.4, SnP, PSLtd, PSQCF)]


returns <- as.xts(allPortfolios[,.(date, `40-60`, SnP)])
...
allPortfolios[, `PS Multi Strategy and S&P500` := `40-60` * 0.5 + SnP * 0.5]
setnames(allPortfolios, c("40-60", "SnP"), c("PS Multi Strategy","S&P500"))
