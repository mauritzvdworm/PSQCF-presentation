...
- Quantitative
    ...
    - Strategies inspired by descretional methodology
...
- ** Carry - inspired by discretionary methodology (BB)1**
...
avgneg <- monthlydata[sign == -1, mean(normalised)]
avgpos <- monthlydata[sign == 1, mean(normalised)]

noneg <- nrow(monthlydata[sign == -1])
nopos <- nrow(monthlydata[sign == 1])


maxdraw <- maxDrawdown(as.xts(BB1returns[, .(date, return)]))[1]
ret.maxdraw <- maxdraw/tbl1[variable == "Annualized Return", BB1]
...
maxdraw <- maxDrawdown(as.xts(BB2returns[, .(date, return)]))[1]
...
portfolio[, cumret := cumsum(return)]


portfolio[, normalised := cumprod(1 + return)]
...
plotdata <- rbind(portfolio[date < portfoliolive[, min(date)],.(date, return, type = "Backtest")], portfoliolive[,.(date, return = PnL, type = "Live")])
plotdata[!is.na(return), cumret := cumsum(return)]
...
tbldata <- rbind(portfolio[,.(date, return)], portfoliolive[, .(date, return = PnL)])
...
avgneg <- monthlydata[sign == -1, mean(cumret)]
avgpos <- monthlydata[sign == 1, mean(cumret)]
...
maxdraw <- maxDrawdown(as.xts(tbldata[, .(date, return)]))[1]
...
maxdraw <- maxDrawdown(as.xts(tbldata[date >= "2008-01-01", .(date, return)]))[1]
ret.maxdraw <- maxdraw/tbl2[variable == "Annualized Return", `2008-`]

avgneg <- monthlydata[sign == -1 & index >= "2008-01-01", mean(cumret)]
avgpos <- monthlydata[sign == 1 & index >= "2008-01-01", mean(cumret)]
...
maxdraw <- maxDrawdown(as.xts(tbldata[date >= "2015-01-01", .(date, return)]))[1]
ret.maxdraw <- maxdraw/tbl3[variable == "Annualized Return", `2015-`]

avgneg <- monthlydata[sign == -1 & index >= "2015-01-01", mean(cumret)]
avgpos <- monthlydata[sign == 1 & index >= "2015-01-01", mean(cumret)]
...
avgrets <- monthlydata[, .(avgret = mean(Return), .N), by = .(Portfolio, sign)]
